...
def render_typo_editor(q, user, context, source_file_by_id):
    ...
    The expander below tracks its own open/closed state in session_state
    (expanded_key) instead of relying on st.expander()'s default -- a
    successful "Save typo fix" triggers st.rerun() (see below) so the
    refreshed question preview above picks up the change immediately, but
    st.expander() resets to closed on every rerun unless told otherwise.
    Without expanded_key, that rerun made the whole panel snap shut the
    instant Save was pressed, wiping the confirmation message before it
    could be read -- easy to misread as "the save didn't do anything",
    especially for the Match-the-following layout where the visible
    effect (the two-column render) is above the fold, outside this
    expander, and easy to miss if the panel appears to have just reset.
    msg_key carries the confirmation/warning/error text across that same
    rerun for the same reason: st.success()/st.warning() called right
    before st.rerun() would otherwise flash for a fraction of a second at
    best and typically never paint at all."""
    expanded_key = f"typofix_expanded_{context}_{q['question_id']}"
    msg_key = f"typofix_msg_{context}_{q['question_id']}"
    pending_msg = st.session_state.pop(msg_key, None)
    with st.expander("✏️ Fix a typo", expanded=st.session_state.get(expanded_key, False)):
        if pending_msg:
            msg_kind, msg_text = pending_msg
        ...
        if st.button("Save typo fix", key=f"typofix_save_{context}_{q['question_id']}"):
            ...
            if "error" in result:
                ...
                # Stashed into session_state (msg_key) rather than shown directly
                # here, since the st.rerun() right below would otherwise wipe it
                # before it ever painted -- see this function's docstring.
                if result["github_ok"] and result["github_log_ok"]:
                    st.session_state[msg_key] = (
                        "success", f"Saved and synced to GitHub: {', '.join(result['changed'])}."
                    )
